...
var nextColorGroupToAssign = 0;
var colorRegistry = {};

function resetColorRegistry() {
	nextColorGroupToAssign = 0;
	colorRegistry = {};
}
function getPlotColor(station, type, depth) {
	var primaryKey = (groupBy == "type" ? station : type);
	var secondaryKey = "d"+depth;
	if(colorRegistry.hasOwnProperty(primaryKey)) {
		if(colorRegistry[primaryKey].hasOwnProperty(secondaryKey)) {
			return colorRegistry[primaryKey][secondaryKey];
		}
		else {
			var colorGroupID = colorRegistry[primaryKey].colorGroupID;
			var nextColorID = colorRegistry[primaryKey].nextColorID;
			var color = plotColorGroups[colorGroupID][nextColorID];
			colorRegistry[primaryKey].nextColorID = (nextColorID + 1)%plotColorGroups[colorGroupID].length;
			colorRegistry[primaryKey][secondaryKey] = color;
			return color;
		}
	}
	else {
		var color = plotColorGroups[nextColorGroupToAssign][0];
		colorRegistry[primaryKey] = {
			colorGroupID: nextColorGroupToAssign,
			nextColorID: 1 // NOTE: this assumes that there are at least two colors in every color group!
		};
		colorRegistry[primaryKey][secondaryKey] = color;
		nextColorGroupToAssign = (nextColorGroupToAssign + 1)%plotColorGroups.length;
		return color;
	}
}
